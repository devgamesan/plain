...
`:` を押すと、利用可能な操作を検索して実行できます。クエリが空の場合は、実効対象（選択、フォーカス、または現在のフォルダ）を説明し、実行可能な `Suggested` 操作を最大5件表示します。カテゴリ名は検索案内として表示され、文字を入力すると全コマンドを検索できます。選択行はキーボードとマウスから操作でき、`↑` / `↓` と `Ctrl+j` / `Ctrl+k` では選択行が常に表示範囲へ追従します。

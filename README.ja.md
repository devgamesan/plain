...
既定では、現在の状態に関連する操作をヘルプバーに表示し、その他の操作はコマンドパレットから検索して実行できます。ファイルの閲覧、プレビュー、検索、grep、置換、2 ディレクトリ間の転送をターミナル内で完結できます。
...
3 ペインでディレクトリを移動しながら右ペインでファイルをプレビューできます。ファイル検索・grep機能により、目的のファイルに素早くジャンプできます。ヘルプバーは現在の状態と標準キーマップに追随するため、迷わず操作できます。
...
既定では、現在の状態に関連する操作が画面下部のヘルプバーに表示されます。

...
| `Rename` / `Rename N items` | 対象が 1 件以上あるとき（Search Workspace を除く） | 1 件なら従来の単一対象リネーム、2 件以上なら Old Name / New Name / Status を確認できる一括リネームオーバーレイを開きます。New Name の直接編集と、リテラル Find/Replace の下書き適用に対応します。Enter / クリックの `Rename items` は全行を再検証してから実行し、衝突・不正名・対象消失などがあれば適用しません。通常ブラウズと Transfer の両方で利用できます。 |

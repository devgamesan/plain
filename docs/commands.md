...
Empty and fallback panes expose only executable local actions: `Create` for an empty current directory, `Clear filter` for zero matches, `Show attributes` for unavailable content previews, and `Edit config` for previews disabled in settings. Fallback actions show `:` as the keyboard route to the command palette (`:` → the displayed command); clicking the inline action is also supported and uses the same reducer flow as the existing command.

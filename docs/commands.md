...
| `Open terminal` | Always | Launches a separate terminal window rooted at zivo's current directory, using `config.toml` templates before built-in fallbacks. Use this for independent or longer-running work. |
...
| Custom actions | Matches each configured action's `when` and `extensions` settings | Shows reusable named entries from `[[actions.custom]]` in `config.toml`. zivo confirms the expanded command before running it. Use `background` for repeatable non-interactive tasks, `terminal` for interactive tasks, and `terminal_window` for independent work. See [Custom Actions](custom-actions.md). |

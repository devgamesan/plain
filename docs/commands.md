...
Empty and fallback panes expose only executable local actions: `Create file` / `Create directory` for an empty current directory, `Clear filter` for zero matches, `Show attributes` for unavailable content previews, and `Edit config` for previews disabled in settings. Clicking these actions uses the same reducer flow as the existing command.

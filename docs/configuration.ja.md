...
| `display` | `enable_pdf_preview` | `true` / `false` | 上限制付きの PDF テキストプレビューを有効にします。`pdftotext` が利用可能なら高速化のため自動的に優先し、ない場合は組み込みの `pypdf` を使います。backend選択用の設定は不要です。既定値は `true` です。無効時は設定無効であること、概要メタデータ、`Edit config` を表示します。 |

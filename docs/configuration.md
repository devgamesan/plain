...
| `display` | `enable_pdf_preview` | `true` / `false` | Enables bounded built-in PDF text extraction through `pypdf`, with an optional `pdftotext` fallback for completed extraction failures. Defaults to `true`. When disabled, the pane reports that preview is disabled and offers `Edit config` with compact metadata. |
| `display` | `enable_office_preview` | `true` / `false` | Enables built-in OOXML text extraction for `docx`, `xlsx`, and `pptx` files. Defaults to `true`. When disabled, the pane reports that preview is disabled and offers `Edit config` with compact metadata. |

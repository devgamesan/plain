...
通常Browsingでは、ヘルプバーの3行構成と項目順を維持します。カーソル、選択、クリップボード、Undoの状態に関連する操作を強調し、実行条件を満たさない操作もキー確認用に残したまま減色します。Browsing、Search Workspace、Transferのヘルプ項目をクリックすると、そのキー入力と同じdispatcherを通ります。実行できない項目をクリックした場合は、必要な条件をステータスバーに表示します。
